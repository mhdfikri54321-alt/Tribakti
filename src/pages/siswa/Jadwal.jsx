...
export default function Jadwal() { 
  ...
  const handleBukaModalPesan = (sesi) => { 
  ...
  const handleSimpanBookingJadwal = async (e) => { 
    ...
    try { 
      ...
      Swal.fire({ 
        icon: 'success', 
        title: 'Berhasil!', 
        text: 'Jadwal latihan Anda telah berhasil dipesan.', 
      ...
      return (
        <div className="bg-white border border-[#e9e9e7] rounded-xl p-6 hover:border-[#0b6e99]/30 transition-all group">
          <div className="flex justify-between items-start mb-4">
            ...
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${dataDb.status === 'Selesai' ? 'bg-emerald-50 text-emerald-600' : dataDb.status === 'Belum Dijadwalkan' ? 'bg-slate-100 text-slate-500' : 'bg-amber-50 text-amber-600'}`}>
              {dataDb.status}
            </span>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            {dataDb.status !== 'Selesai' && (
              dataDb.status === 'Belum Dijadwalkan' ? (
                ...
                <div className="text-center py-2 text-[9px] font-bold text-[#37352f]/30 uppercase tracking-widest border border-[#efefed] rounded-lg">
                  Jadwal Terkunci
                </div>
              ) : (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl relative z-10 flex flex-col border border-[#e9e9e7]">
            <div className="p-6 border-b border-[#e9e9e7] flex justify-between items-center bg-[#fbfbfa]">
              <h3 className="text-lg font-bold">Booking Pertemuan {selectedSesi?.pertemuan_ke}</h3>
