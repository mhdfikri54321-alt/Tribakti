...
export default function AdminLaporan() {
  ...
  const [rawData, setRawData] = useState([]); // Data mentah untuk tabel & ekspor
  ...
  const [filterBulan, setFilterBulan] = useState('Semua'); // State filter bulan
  ...
  const COLORS = ['#0b6e99', '#37352f']; // Teal and Dark Gray for Notion-style
  ...
  return (
    <div className="bg-[#fbfbfa] min-h-screen flex text-[#37352f] font-sans">
      <div className="print:hidden">
        <AdminSidebar role="admin" activeMenu="laporan" />
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 print:hidden">
        ...
        <main className="flex-1 overflow-y-auto w-full px-4 md:px-8 py-6 md:py-12">
          <div className="mb-8 md:mb-12 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 sm:gap-6">
            <div>
              ...
              <h2 className="text-2xl md:text-4xl font-bold tracking-tight mb-3 md:mb-4 leading-tight">
                Laporan <span className="text-[#37352f]/40">Pertumbuhan.</span>
              </h2>
              <p className="text-[#37352f]/70 text-sm md:text-lg max-w-2xl leading-relaxed font-medium">
                Tinjauan mendalam performa bisnis, tren pendaftaran, dan arus kas Tri Bakti.
              </p>
            </div>
          </div>

          <AdminTabSwitcher group="laporan" activeTab="bisnis" />
          ...
          {/* Laporan Distribusi Paket Kursus - Full Width */}
          <div className="bg-white rounded-2xl border border-[#e9e9e7] shadow-sm mb-10 overflow-hidden">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-[#fbfbfa] border-b border-[#e9e9e7]">
                    ...
                    <th className="px-8 py-4 text-[10px] font-bold uppercase tracking-widest text-[#37352f]/40 text-right">Pengeluaran Ops</th>
                    ...
                    financialSummary.map((item, idx) => (
                      <tr key={idx} className="text-sm hover:bg-[#fbfbfa] transition-colors">
                        <td className="px-8 py-4">
                          <button
                            onClick={() => navigate(`/admin/laporan/detail?periode=${encodeURIComponent(item.label)}&type=bulanan`)}
          ...
          <div className="mt-8 md:mt-12">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 md:mb-8 gap-4">
              <div>
                <h4 className="text-xl md:text-2xl font-bold text-[#37352f] tracking-tight flex items-center gap-2">
                  <BarChart3 className="w-5 h-5 md:w-6 md:h-6 text-[#0b6e99]" /> Histori Transaksi
                </h4>
                <p className="text-[9px] md:text-[10px] font-bold uppercase text-[#37352f]/40 tracking-widest mt-1">Data Lengkap Histori Pendaftaran</p>
              </div>
            ...
            <div className="bg-white border border-[#e9e9e7] rounded-2xl overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-[#fbfbfa] border-b border-[#e9e9e7]">
                      <th className="px-6 py-5 text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-[#37352f]/40">Tanggal</th>
                      <th className="px-6 py-5 text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-[#37352f]/40">Siswa</th>
                      <th className="px-6 py-5 text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-[#37352f]/40">Paket</th>
                      <th className="px-6 py-5 text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-[#37352f]/40">Pemasukan</th>
                      <th className="px-6 py-5 text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-[#37352f]/40 text-center">Status</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#e9e9e7]">
                    {loading ? (
                      <tr>
                        <td colSpan="5" className="px-6 py-12 text-center text-[#37352f]/40 font-medium text-xs md:text-sm">Memuat data transaksi...</td>
                      </tr>
                    ) : displayedHistoriData.length === 0 ? (
                      <tr>
                        <td colSpan="5" className="px-6 py-12 text-center text-[#37352f]/40 font-medium text-xs md:text-sm">Tidak ada data untuk bulan {historiBulan}.</td>
                      </tr>
                    ) : (
                        ...
                        <tr key={item.id} className="hover:bg-[#fbfbfa] transition-colors">
                          <td className="px-6 py-5">
                            <div className="flex flex-col">
                              <span className="text-xs md:text-sm font-semibold text-[#37352f]">{new Date(item.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                              <span className="text-[9px] md:text-[10px] font-medium text-[#37352f]/40 uppercase tracking-tighter">{new Date(item.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })} WIB</span>
                            </div>
                          </td>
                          <td className="px-6 py-5">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex flex-col min-w-0">
                                <span className="text-xs md:text-sm font-semibold text-[#37352f] truncate">{item.nama_lengkap}</span>
                                <span className="text-[9px] md:text-[10px] font-medium text-[#37352f]/40 truncate">{item.no_whatsapp}</span>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-5">
                            <span className="text-[10px] md:text-xs font-semibold text-[#37352f] bg-[#efefed] px-2 py-1 rounded-md border border-[#e9e9e7]">{item.paket_pilihan}</span>
                          </td>
                          <td className="px-6 py-5">
                            <span className="text-xs md:text-sm font-bold text-[#37352f]">Rp {Number(item.total_bayar).toLocaleString('id-ID')}</span>
                          </td>
                          <td className="px-6 py-5 text-center">
                            <span className={`inline-flex px-2.5 py-1 rounded-full text-[9px] md:text-[10px] font-bold uppercase tracking-widest border ${item.status === 'Berhasil'
      ...
      <div className="print-section-keuangan bg-white text-black p-8 font-sans w-full max-w-[21cm] min-h-[29.7cm] border border-black/10 mx-auto text-xs">
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <table className="w-full text-left border-collapse border-t border-b border-l border-r border-gray-300">
              <thead>
